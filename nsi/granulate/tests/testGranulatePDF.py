...
"""
Test methods GranulatePDF
"""

import sys, unittest
import re, time,os
from nsi.granulate import File
from nsi.granulate import GranulatePDF

absPath = os.path.abspath(__file__)

# FileInfo{Filename:(Image Number, Table Number)}          13 6
FileInfo = {'1image6tables.pdf':(1,6),'Monografia.pdf':(26,13)}
...
class TestGranulatePDF(unittest.TestCase):

    def createInstanceGraPdf(self, filename, filedata):
        fileDoc = File(data=filedata,filename=filename)
        granObj = GranulatePDF(fileDoc)
        return granObj

    def testCheckCreateInstanceGranPdf(self):
        """
        ...
        for k, v in dictFile.iteritems():
            granObj = self.createInstanceGraPdf(filename = k, filedata = v)
    ...
    def testGetSummaryDocument(self):
        ...
        for k, v in dictFile.iteritems():
            ...
            self.failUnless(resultList is None)
            #self.assertEquals(resultDict['summary'], None)
            print " \n File: "+ k + '\n Time run method: ' + str((time.time() -t1)/(24*3600))
    ...
    def testAllMethodsGranulateObj(self):
        ...
        for k, v in dictFile.iteritems():
            ...
            self.failUnless(resultListSummary is None)
